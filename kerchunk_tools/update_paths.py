...
def rewrite_kerchunk_parquet(
    input_parquet_path, output_parquet_path, old_prefix, new_prefix
):
    ...
    for store in parquet_dir.glob("*"):
        if store.is_dir():
            for parquet_file in store.glob("*.parq"):
                rewrite_kerchunk_parquet(
                    parquet_file, parquet_file, old_prefix, new_prefix
                )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Rewrite path prefixes in all kerchunk parquet stores in a directory."
    )
    parser.add_argument(
        "input_dir", help="Directory containing kerchunk parquet stores"
    )
